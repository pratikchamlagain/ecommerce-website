...
import { fetchPaymentHistory } from "../lib/paymentsApi";

export default function PaymentHistoryPage() {
  const historyQuery = useQuery({
    queryKey: ["payment-history", "customer"],
    queryFn: () => fetchPaymentHistory({ page: 1, limit: 20 })
  });

  const items = historyQuery.data?.items || [];

  return (
    <PageShell title="My Payments">
      {historyQuery.isPending ? <p className="wm-muted">Loading payment history...</p> : null}
      {historyQuery.isError ? <p className="text-rose-600">Could not load payment history.</p> : null}

      {!historyQuery.isPending && !historyQuery.isError && items.length === 0 ? (
        <p className="wm-muted">No payment records yet.</p>
      ) : null}
